...
// Function to calculate the number of sets for approximately 2GB total memory
int calculate_n_sets_runtime() {
    ...
    // Calculate memory per matrix set using runtime values
    const size_t matrix_m = g_mat_m;
    const size_t matrix_n = g_mat_n;
    const size_t matrix_k = g_mat_k;

    const size_t size_a = matrix_m * matrix_k * sizeof(fp16); // fp16
    const size_t size_b = matrix_k * matrix_n / 16 * sizeof(int2x16); // int2x16 = 2 bytes (16 values packed)
    const size_t size_c = matrix_m * matrix_n * sizeof(fp16); // fp16
    const size_t size_acc = matrix_m * matrix_n * sizeof(int32_t); // 4 bytes
    const size_t size_cnt = matrix_m * matrix_n * sizeof(uint32_t); // 4 bytes

    const size_t memory_per_set = (size_a + size_b + size_c + size_acc + size_cnt);

    // Calculate number of sets to reach target memory
    const int n_sets = static_cast<int>((target_memory_bytes + memory_per_set - 1) / memory_per_set);

    // Return computed number of sets (ensure at least 1)
    return n_sets > 0 ? n_sets : 1;
}
...
void int2_dequantize_gemm_run(int iter, int n_sets, bool enable_validation) {
    ...
    using compute_policy = xetla::group::compute_policy_int2_fp16_dpas_xmx<compute_attr, perf_tuning_knob, float, float, static_cast<int>(Test::mma_xmx_m), arch_tag>;
...
#pragma omp parallel for
        ...
        for (int rowA = 0; rowA < matrix_m; rowA++) {
            for (int igs = 0; igs < scale_gs; igs++) {
                ...
                ScaleA_h[set][igs * matrix_m + rowA] = 127.0f / absmax_row;
            }
        }
...
// Provides both scale modes (external/internal) and all 4 post-op combos.
