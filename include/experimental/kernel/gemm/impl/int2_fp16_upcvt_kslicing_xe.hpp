...
/// C++ API
///
/// Kernel-level (workgroup) orchestrator for int2 weight-only GEMM with fp16
...
///   B is stored as int2x16 (uint32_t) packed along the K axis.
...
class gemm_universal_t<dispatch_policy_int2_fp16_upcvt_kslicing<group_swizzle_,
    ...
    static_assert(gemm_t::compute_policy::is_int2_matB_policy,
            "should match with int2 gemm impl");
...
public:
    ...
    static bool can_implement(arguments_t &args) {
        ...
        // K must be a multiple of pack_ratio (16) for clean packing.
