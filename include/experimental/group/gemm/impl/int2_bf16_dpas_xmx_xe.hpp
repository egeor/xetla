...
/// @brief Is the gemm functor for Xe architecture with DPAS instruction for int2_bf16.
...
class gemm_t<compute_policy_int2_bf16_dpas_xmx<compute_attr_, perf_tuning_knob_, dtype_scale_a_, dtype_scale_b_, mma_xmx_m_, arch_tag_>,
               ...
               std::enable_if_t<(arch_tag_ == gpu_arch::Xe)
                       >> {
public:
    using compute_policy = compute_policy_int2_bf16_dpas_xmx<compute_attr_, perf_tuning_knob_, dtype_scale_a_, dtype_scale_b_, mma_xmx_m_, arch_tag_>;
...
#pragma unroll
    ...
    // Unified templated overload to support float and bf16 output tiles without duplicating the implementation
    ...
    __XETLA_API KERNEL_FUNC void operator()(work_group_t &g, subgroup::tile_t<OutT, matAcc_tile_desc_t> &matAcc_f, ArgsT args, uint32_t slm_base = 0, uint32_t nbarrier_base = 0) {
        static_assert(std::is_same_v<OutT, float> || std::is_same_v<OutT, bf16>, "Unsupported output tile element type");
        int32_t sg_idx = g.get_id() % wg_size_x;
        int32_t sg_idy = g.get_id() / wg_size_x;
        update_sg_tile_tdesc(args, sg_idx, sg_idy);
...
#pragma unroll
        ...
        for (int igk = 0; igk < k_groups; igk++) {
            ...
            for (int j = 0; j < k_chunks_minus_stages; j++) {
                ...
                matB_acc_t matB_acc;
                //subgroup::elemwise_cvt(matA_acc, matA);
                subgroup::elemwise_scale_bf16_to_int8(matA_acc, matA, scaleA_tile);
                subgroup::elemwise_cvt(matB_acc, matB);
                SW_BARRIER();
