        ...
        for (int j = 0; j < N; ++j) {
            ...
            for (int gk = 0; gk < ks_groups; ++gk) {
                ...
                for (int p = 0; p < scale_gs; ++p) {
                    ...
                    uint32_t word = B_packed[(ik / 32) * ldb + j];
                    uint32_t code = (word >> (ik % 32)) & 0x1u;
                    int bv = int1_code_to_value(code);
...
void run_gemm_impl_inner(const RunConfig &cfg) {
    ...
    using compute_policy
            = gpu::xetla::group::compute_policy_int1_fp16_upcvt_xmx<compute_attr,
    ...
    const size_t size_b_words
            = static_cast<size_t>(K / 32) * ldb_padded; // int1x32 words
...
#pragma omp parallel for
        ...
        // B int1x32 codes: random uniform bits. 0 -> +1, 1 -> -1.
        // Balanced sign distribution.
#pragma omp parallel for
        for (size_t i = 0; i < size_b_words; ++i) {
            B[i].data = random_uint32();
        }
